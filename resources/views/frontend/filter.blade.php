...
<div class="container container-filter">
    ...
    {{-- @endforeach --}}
    <div class="uri-view-filter"> <strong class="url-in-filter"><a href="{{ route('menu') }}"><strong
                    class="url-in-filter">Beranda</strong></a> > Kategori Utama > Kategori Roti ></strong>
        ...
        <div class="container-filter-dan-menu">
            <div class="container-filter-intrac">
                ...
                <div class="content-filter-intrac">
                        <input type="number" class="input-filter" name="hargaMinimum" placeholder="Harga Min"
                            id="harga_minim">
                        <input type="number" class="input-filter" name="hargaMaximum" placeholder="Harga Max"
                            id="marga_maximum">
                        ...
                        <button type="button" id="filter" style="width: 100%; border-radius:10px;"
                            class="btn  button btn-bayar input-filter">
                            <span>Filter</span>
                        </button>
                </div>
            </div>
            ...
            <div class="containerCard menu-card">
                @foreach ($menu as $mFilter)
                    <a href="{{ route('invoice.menu', $mFilter->id_roti) }}" class="card card-margin">
                        <div class="cardimage">
                            <img class="imageProduct" src="{{ asset('gallery/' . $mFilter->gambar) }}" width="186"
...
{{-- filter --}}
<script>
    var btnFiltering = document.getElementById('filter');
    var hargaMinimum = document.getElementById('harga_minim');
    var hargaMaximum = document.getElementById('marga_maximum');
    
    btnFiltering.addEventListener('click', function(event) {
        
        // konfersi bilangan 
        var min = parseInt(hargaMinimum.value);
        var max = parseInt(hargaMaximum.value);

        // get card menu
        var menuItems = document.querySelectorAll('.card');

        menuItems.forEach(function(menuItem){
            var hargaReal = menuItem.querySelector('.hargaProduct');
            var harga = parseInt(hargaReal.innerText.replace(/[^0-9]/g, ''))
